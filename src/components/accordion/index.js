...
import {Container, Inner, Title, Header, Item, Body} from './styles/accordion'
...
Accordion.Header = function AccordionHeader({children}) {
    ...
    return (
        <Header onClick={() => setToggleShow(toggle => !toggle)}>{children}
...
Accordion.Body = function AccordionBody({children}) {
    ...
    return (
        toggleShow ? <Body>{children}</Body> : null
    )
}
